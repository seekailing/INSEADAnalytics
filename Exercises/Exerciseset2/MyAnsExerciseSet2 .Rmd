...
2. 21.92%
<br>
3. These 10 factors will represent 60.61% of population. These 10 factors are namely 
C1:Risk Free rates (Tbills) of US&UK
C2 Strength of E-rates of Developed Countries of Euro, Swiss Franks, Canadian Dollars, Pound, Norwegian Krone, SGD, NZ, AUD, JPY & Gold and Silver Prices
C3: EXchange Indexes of Developed European Countries FTSE, STockholm, France, DAX, Eurostoxx, SP
C4: Exchange Indexes of Others: Toronto, Nasdaq, S&P500, DJ
C5: Eurobonds
C6: Oil Prices
C7: Metal Prices of Copper, Aluminium, Zinc, Nickel
C8: Food Commodities of Soybeans, Wheat, Soybean Oil, Corn, Canola Oil
C9: Precious Metal Prices of Gold, Silver, Palladium, Platinum
C10: Major Asian Stock Market: Nikkei and HangSeng
...
Variance_Explained_Table_CD <- cbind(paste("component", 1:ncol(crisis_data), sep = " "), 
    Variance_Explained_Table_results_CD$eig)
Variance_Explained_Table_CD <- as.data.frame(Variance_Explained_Table_CD)
colnames(Variance_Explained_Table_CD) <- c("Component", "Eigenvalue", "Percentage_of_explained_variance", 
    "Cumulative_percentage_of_explained_variance")
```

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data = data.frame(Variance_Explained_Table_CD)
m1<-gvisTable(show_data,options=list(width=1920, height=min(400,27*(nrow(show_data)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
### Obtained Scree Plot    
...
Line         <- gvisLineChart(as.data.frame(df_CD), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line, 'chart')
```


###Determine Rotated Factor for Crisis Data

```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
corused = cor(crisis_data[, apply(crisis_data != 0, 2, sum) > 10, drop = F])
Rotated_Results_CD <- principal(corused, nfactors = 20, rotate = "varimax", score = TRUE)
...
sorted_rows <- sort(Rotated_Factors_CD[, 1], decreasing = TRUE, index.return = TRUE)$ix
...
# Conduct Principle Component Analysis for Fin Data

```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
fin_data = futures_data[as.Date(rownames(futures_data)) > "2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]
Variance_Explained_Table_results_FD <- PCA(fin_data, graph = FALSE)
Variance_Explained_Table_FD <- cbind(paste("component", 1:ncol(fin_data), sep = " "), 
    Variance_Explained_Table_results_FD$eig)
Variance_Explained_Table_FD <- as.data.frame(Variance_Explained_Table_FD)
colnames(Variance_Explained_Table_FD) <- c("Component", "Eigenvalue", "Percentage_of_explained_variance", 
    "Cumulative_percentage_of_explained_variance")
```

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data_FD = data.frame(Variance_Explained_Table_FD)
m1<-gvisTable(show_data_FD,options=list(width=1920, height=min(400,27*(nrow(show_data_FD)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
# Obtained Scree Plot    
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
eigenvalues_FD <- Variance_Explained_Table_FD[, 2]
```

```{r Fig3, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
df_FD           <- cbind(as.data.frame(eigenvalues_FD), c(1:length(eigenvalues_FD)), rep(1, length(eigenvalues_FD)))
colnames(df_FD) <- c("eigenvalues", "components", "abline")
Line         <- gvisLineChart(as.data.frame(df_FD), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line, 'chart')
```


#Determine Rotated Factor for Fin Data
```{r echo=TRUE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
corused_FD = cor(fin_data[, apply(fin_data != 0, 2, sum) > 10, drop = F])
Rotated_Results_FD <- principal(corused, nfactors = 20, rotate = "varimax", score = TRUE)
Rotated_Factors_FD <- round(Rotated_Results_FD$loadings, 2)
Rotated_Factors_FD <- as.data.frame(unclass(Rotated_Factors_FD))
colnames(Rotated_Factors_FD) <- paste("Component", 1:ncol(Rotated_Factors_FD), sep = " ")

sorted_rows_FD <- sort(Rotated_Factors_FD[, 1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors_FD <- Rotated_Factors_FD[sorted_rows_FD, ]
Rotated_Factors_FD[abs(Rotated_Factors_FD) < 0.3] <- NA
```

```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
show_data_FD <- Rotated_Factors_FD 
show_data_FD <-cbind(rownames(show_data_FD),show_data_FD)
colnames(show_data_FD)<-c("Variables",colnames(Rotated_Factors_FD))
m1<-gvisTable(show_data_FD,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data_FD)+1)),allowHTML=TRUE,page='disable'))
print(m1,'chart')
```
...
5. The Principle components that can explain the Bull and Bear markets differ. When the market is Bullish, its indicative factors are spreadout across more components. whereas for a Bearish market, it appeared that Risk Free Rates of US & UK, Currenecy of Major Developed European countries and Exchange INdexes of Develop European Countries would explain majority of the performance
...
1. The code takes the 13 length(MAfreq) historical moving averages of each futures compoenent and determine the direction (sign) of those moving averages. These moving averages then indicate whether we should short sell or long (buy) that future today (as the assumption here is that historical moving averages gives an indication of how the future will move). The amount to long or short the stock is given by the relative weights.

2. IF we use different moving aaverages technical indiators in the code (i.e by increasing number of moving average using command line MAfreq<-seq(10,250,by=5)) we will take more 'sample' averages but it will still give us similar trading strategy that move in the same (upward) direction
